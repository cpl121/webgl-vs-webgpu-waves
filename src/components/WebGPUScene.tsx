...
import {
  uv, time, texture, float, vec2, vec3, add, sub, mul, pow, abs,
  mix, smoothstep, select, lessThan, positionLocal, sin, cos, div
} from 'three/tsl'
...
export default function WebGPUCanvas() {
  ...
  useEffect(() => {
    ...
    const rotate2D = (val: THREE.TSL.ShaderNodeObject<THREE.Node>, ang: THREE.TSL.ShaderNodeObject<OperatorNode>) => {
    ...
    function createFire(perlin: THREE.Texture) {
      ...
      const pos = positionLocal, t = time
      ...
      mat.positionNode = vec3(add(pos.x, add(offset, mul(shape, waveX))), pos.y, add(pos.z, add(offset, mul(shape, waveZ))))

      const uvNode = uv(), noiseUV = mul(add(uvNode, vec2(float(0), mul(t, float(0.2)))), float(2.5))
      ...
      const mask = mul(mul(smoothstep(float(0.5), float(0.25), div(d, add(sub(float(1.0), uvNode.y), float(0.01)))), smoothstep(float(0.02), float(0.15), uvNode.y)), smoothstep(float(0.95), float(0.7), uvNode.y))

      const color = select(lessThan(y, float(0.2)),
        mix(vec3(0.1, 0, 0), vec3(1, 0.2, 0), div(y, float(0.2))),
        select(lessThan(y, float(0.6)),
    ...
    function createSmoke(perlin: THREE.Texture) {
      ...
      const pos = positionLocal, uvV = uv(), t = time
      ...
      mat.positionNode = vec3(add(twistXZ.x, mul(windX, windStr)), pos.y, add(twistXZ.y, mul(windZ, windStr)))
    ...
    function createLight() {
      ...
      flickeringLight.position.set(flickeringLight.position.x + 1.25, 0.5, flickeringLight.position.z + 1.25)
